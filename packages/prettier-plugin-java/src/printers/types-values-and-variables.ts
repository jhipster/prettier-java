...
export default {
  primitiveType(path, print) {
    ...
    return join(" ", [...annotations, call(path, print, typeKey)]);
  },
  ...
  typeVariable(path, print) {
    const annotations = path.node.children.annotation
      ? map(path, print, "annotation")
      : [];
    return join(" ", [...annotations, call(path, print, "Identifier")]);
  },
  ...
  dims(path, print) {
    return flatMap(
      ...
      childPath => {
        ...
        return isNode(childPath.node) ? [child, " "] : child;
      },
  ...
  typeParameter(path, print) {
    const { children } = path.node;
    const parameter = children.typeParameterModifier
      ? map(path, print, "typeParameterModifier")
      : [];
    parameter.push(call(path, print, "typeIdentifier"));
    if (children.typeBound) {
  ...
  wildcard(path, print) {
    const { children } = path.node;
    const wildcard = children.annotation ? map(path, print, "annotation") : [];
    wildcard.push("?");
    if (children.wildcardBounds) {
