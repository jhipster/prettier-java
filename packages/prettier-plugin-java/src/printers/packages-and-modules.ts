...
export default {
  ...
  packageDeclaration(path, print) {
    const modifiers = path.node.children.packageModifier
  ...
  moduleDeclaration(path, print) {
    ...
    const declarations = children.moduleDirective
      ? map(
          path,
          declarationPath => {
  ...
  requiresModuleDirective(path, print) {
    const modifiers = path.node.children.requiresModifier
        ...
        indent([
          ...
          group(
            indent([
              "to",
              line,
              ...join([",", line], map(path, print, "moduleName"))
            ])
          )
        ])
      )
