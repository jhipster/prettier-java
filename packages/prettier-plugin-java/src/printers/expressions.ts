...
export default {
  ...
  lambdaExpression(path, print, _, args) {
    const hug = (args as { hug?: boolean } | undefined)?.hug ?? false;
  ...
  lambdaParametersWithBraces(path, print, options) {
    ...
    if (parameterCount > 1) {
      return group(["(", indent([softline, parameters]), softline, ")"]);
    }
  ...
  unaryExpression(path, print) {
    const { children } = path.node;
    const prefixOperators = children.UnaryPrefixOperator
      ? map(path, print, "UnaryPrefixOperator")
      : [];
    const suffixOperators = children.UnarySuffixOperator
  ...
  fqnOrRefType(path, print, _, args) {
    ...
    const { children } = path.node;
    const fqnOrRefType = [call(path, print, "fqnOrRefTypePartFirst")];
    if (children.fqnOrRefTypePartRest) {
      path.each(
        partPath => {
          const part = print(partPath);
          if (partPath.isLast) {
            fqnOrRefType.push([
              willBreak(part) ? hardline : lastSeparator,
              part
            ]);
          } else {
            fqnOrRefType.push(part);
          }
        },
        "children",
  ...
  fqnOrRefTypePartRest(path, print) {
    ...
    const type = children.typeArguments
      ? [call(path, print, "typeArguments"), common]
      : common;
    return [".", ...join(" ", [...annotations, type])];
  },
  ...
  parenthesisExpression(path, print) {
    ...
    const ancestorName = (path.getParentNode(13) as JavaNode | null)?.name;
    const binaryExpression = path.getParentNode(7) as JavaNode | null;
    const separator =
      ancestorName &&
      ...
      Object.keys(binaryExpression.children).length === 1
        ? softline
        : "";
    return group(["(", indent([separator, expression]), separator, ")"]);
  },
  ...
  unqualifiedClassInstanceCreationExpression(path, print) {
    ...
    if (children.argumentList) {
      expression.push(group(["(", call(path, print, "argumentList"), ")"]));
    } else {
      expression.push("()");
    }
  ...
  classOrInterfaceTypeToInstantiate(path, print) {
    const { children } = path.node;
    const identifiers = children.annotation
      ? flatMap(
          path,
          childPath => [print(childPath), isNode(childPath.node) ? " " : "."],
          ["annotation", "Identifier"]
        )
      : printName(path, print);
    return children.typeArgumentsOrDiamond
      ? [...identifiers, call(path, print, "typeArgumentsOrDiamond")]
      : identifiers;
  },
  ...
  classLiteralSuffix(path, print) {
    const { children } = path.node;
    const lSquares = children.LSquare ? map(path, print, "LSquare") : [];
    const rSquares = children.RSquare ? map(path, print, "RSquare") : [];
    return [
  ...
  recordPattern(path, print) {
    const patterns = path.node.children.componentPatternList
      ? group([
          "(",
          indent([softline, call(path, print, "componentPatternList")]),
          softline,
          ")"
        ])
      : "()";
...
>(path: AstPath<T>, print: JavaPrintFn, beginKey: C, midKey: C, endKey: C) {
  ...
  const mids = children[midKey as keyof typeof children]
    ? map(path, ({ node }) => node.image, midKey)
    : [];
